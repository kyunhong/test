...
export default function StudentPage() {
  ...
  const loadStudentsList = async (examName) => {
    ...
    const res = await api.get(
      `/analysis/students-list?exam_name=${encodeURIComponent(examName)}&session_id=${sessionId}`  // ✅ session_id 추가
    );
  ...
  const searchStudent = async (studentName) => {
    ...
    const sessionId = getSessionId();  // ✅ session_id 가져오기
    const res = await api.get(
      `/analysis/student?name=${encodeURIComponent(studentName)}&session_id=${sessionId}`  // ✅ session_id 추가
    );
    setData(res.data);
  };
      ...
      <div style={styles.main}>
        ...
        <div style={styles.searchRow}>
          ...
          <button style={styles.btn}
            onClick={() => searchStudent(searchInput)}>검색</button>
        </div>

        {data.length > 0 && (
          <>
            <h3 style={styles.studentName}>
              {name} ({data[0]?.ban}반 {data[0]?.number}번)
            </h3>
            ...
            <div style={styles.chartBox}>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={data}>
                  ...
                  <YAxis domain={[1, 9]} ticks={[1,2,3,4,5,6,7,8,9]} />
                  <Tooltip />
                  <Legend />
                  {subjects.map(s => (
                ...
                <tbody>
                  {data.map((d, i) => (
                    <tr key={i} style={i % 2 === 0 ? {} : { background: '#f9fafb' }}>
                      ...
                      <td style={{ ...styles.td,
                        color: d.korean_grade === 1 ? '#dc2626' : 'inherit',
                        fontWeight: d.korean_grade <= 2 ? 'bold' : 'normal'
                      }}>{d.korean_grade ?? '-'}</td>
                      <td style={styles.td}>{d.math_std ?? '-'}</td>
                      <td style={{ ...styles.td,
                        color: d.math_grade === 1 ? '#dc2626' : 'inherit',
                        fontWeight: d.math_grade <= 2 ? 'bold' : 'normal'
                      }}>{d.math_grade ?? '-'}</td>
                      <td style={{ ...styles.td,
                        color: d.english_grade === 1 ? '#dc2626' : 'inherit',
                        fontWeight: d.english_grade <= 2 ? 'bold' : 'normal'
                      }}>{d.english_grade ?? '-'}</td>
                      <td style={styles.td}>{d.society_std ?? '-'}</td>
                      <td style={{ ...styles.td,
                        color: d.society_grade === 1 ? '#dc2626' : 'inherit'
                      }}>{d.society_grade ?? '-'}</td>
                      <td style={styles.td}>{d.science_std ?? '-'}</td>
                      <td style={{ ...styles.td,
                        color: d.science_grade === 1 ? '#dc2626' : 'inherit'
                      }}>{d.science_grade ?? '-'}</td>
                      <td style={{ ...styles.td,
                        color: d.history_grade === 1 ? '#dc2626' : 'inherit'
                      }}>{d.history_grade ?? '-'}</td>
                    </tr>
                  ))}
                </tbody>
...
const styles = {
  ...
  sidebar:         { width: '220px', minWidth: '220px', background: 'white', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', overflowY: 'auto' },
  sideHeader:      { padding: '16px', fontWeight: 'bold', fontSize: '15px', color: '#1e40af', borderBottom: '1px solid #e5e7eb', background: '#eff6ff' },
  ...
  studentItem:     { padding: '10px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', borderBottom: '1px solid #f3f4f6' },
  studentItemActive:{ background: '#eff6ff', color: '#1e40af', fontWeight: 'bold' },
  numBadge:        { fontSize: '11px', color: '#6b7280', background: '#f3f4f6', borderRadius: '4px', padding: '1px 5px', minWidth: '24px', textAlign: 'center' },
